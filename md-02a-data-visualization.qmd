...
5.  What are the data types of the three variables used for aesthetic mappings?
    ...
    ggplot(data = gapminder_2007,
           mapping = aes(x = continent,
                         y = lifeExp,
                         fill = continent)) +
      #solution for exlcuding outliers from boxplot found at https://stackoverflow.com/questions/5677885/ignore-outliers-in-ggplot2-boxplot
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    -   `geom_bar()` plots the count of each x variable; `geom_col()` plots the values mapped to y (in this case `count` hence the charts are the same)\
        Answer checked via: <https://ggplot2.tidyverse.org/reference/geom_bar.html>
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    -   `geom_histogram()` plots the count of observations in each bin
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    -   `breaks` lets you control the appearance of the plot

    -   the first and second arguments in `seq()` set the upper and lower limits of the x-axis

    -   the third argument sets the width of the bins
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    -   `country_colors` is a list of manually assigned hex color codes for each country in the `gapminder` dataset.
...
world |> 
  ...
  geom_sf() +
  #added line to use colorblind friendly palet. Also loaded ggthemes() in first code block
